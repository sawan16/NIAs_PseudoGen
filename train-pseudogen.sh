...
TRAINING_LINE2=`expr  $ALL_DATA_LINE - 1000`
echo "making data ... " 1>&2
#shuf all.combined > combined.shuf
cat all.combined | head -n $TRAINING_LINE > train.combined
cat all.combined | head -n $TRAINING_LINE2 | tail -1000 > dev.combined
cat all.combined | tail -n 1000 > test.combined
